...
describe("parser/property", function () {
    ...
    it("should parse properties", function () {
        ...
        const template = funclate`<p><input p:type="number" p:placeholder="${placeholder}" ></p>`
        template.render(el)
    ...
    it("should parse complex properties", function () {
        const foo = "foo"
        const bar = "bar"
        const template = funclate`<p><input p:type="text" p:name="${foo}" p:value="before ${bar} after" ></p>`
        template.render(el)
